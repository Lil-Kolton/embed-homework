...
void CenctrlAbilitySlice::SetUpFirstView()
{
    ...
    SetUpItem("主卧空调", RES_AIR_COND_UNSELECT, layout,"Motor", RES_AIR_COND_SELECT);
    SetUpItem("主卧筒灯", RES_LIVINGROOM_UNSELECT, layout,"Led", RES_LIVINGROOM_SELECT);
...
void CenctrlAbilitySlice::InitSensorDisply()
{
    displayCallBackParam callParam;
    callParam.SensorDisplay = this->DisplayeSensor;
    RegisterSensorDisplsyCallback(&callParam);
    CtrlCallBack::SetInstancePram(&itemListeners_);
}

void CenctrlAbilitySlice::InitLamp()
{
    lampCallBackParam callParam;
    callParam.On = CtrlCallBack::CtrlOn;
    callParam.Off = CtrlCallBack::CtrlOff;
    RegisterLampCallback(&callParam);
    CtrlCallBack::SetInstancePram(&itemListeners_);
}
