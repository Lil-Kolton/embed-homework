...
#define MAX_LIST_ITEM 4
namespace OHOS {
const char* g_strPageName[MAX_LIST_ITEM] = {"首页", "灯光", "空调", "窗帘"};
static UILabel* g_pLabelButton[MAX_LIST_ITEM] = {nullptr};

class ClickListener : public UIView::OnClickListener {
public:
    ClickListener(UIViewGroup* view, int index,  UISwipeView* swipeView)
    {
...
public:
    SwipeListener(UIViewGroup* view,  UISwipeView* swipeView)
        : view_(view), swipeView_(swipeView){}
    ~SwipeListener(){}
...
protected:
    ...
    static int DisplayeSensor(int temp, int lux, int hum);
private:
